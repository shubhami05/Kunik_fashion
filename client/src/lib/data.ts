...
export const getCategories = async () => {
  const response = await fetch(`${import.meta.env.VITE_APP_SERVER_URI}/api/categories`);
...
export const updateCategory = async (id: string, updates: Partial<Category>) => {
  try {
    const response = await fetch(`${import.meta.env.VITE_APP_SERVER_URI}/api/categories/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
export const deleteCategory = async (id: string) => {
  try {
    await fetch(`${import.meta.env.VITE_APP_SERVER_URI}/api/categories/${id}`, {
      method: 'DELETE'
    });
  } catch (error) {
